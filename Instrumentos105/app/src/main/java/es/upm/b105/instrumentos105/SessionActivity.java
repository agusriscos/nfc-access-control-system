...
public class SessionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        pref = SharedPreferenceUtils.getInstance(SessionActivity.this);
        original();
        ...
        env.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ventana = true;
                us = intUsuario.getText().toString().toLowerCase();
                cont = intCont.getText().toString();
                if(sonASCII(cont)) {
                    cuadro.show();
                    Comprobar comu = new Comprobar();
                    comu.execute(us, cont);
                }else{
    ...
    private class Comprobar extends AsyncTask<String, Void, InputStream> {
        ...
        protected void onPostExecute(InputStream stream) {
            /*DEBUG for reader: QUITAMOS COMPROBACION CON EL SERVIDOR
            pref.setValue("username",user);
            String userName = pref.getValue("username","UK");
            Log.i("TAG", "Se ha guardado el usuario " + userName);
            ...
            if (stream != null) {
                ...
                Log.i("input",datosEntrada);
                if(datosEntrada.equals("OK\n")&& ventana) {
                    pref.setValue("username",user);
                    String userName = pref.getValue("username","UK");
                    Log.i("CHECK", "Se ha guardado el usuario " + userName + " en preferencias compartidas");
                    Intent dev = new Intent();
                    dev.putExtra(Utils.USER_SESSION, user);
            ...
            } else { //si el stream de entrada es nulo...
